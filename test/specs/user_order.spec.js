...
describe("Select products and place orders", () => {
  ...
  it("Complete an order", async () => {
    ...
    await browser.step(
      ...
      async function () {
        await productsPage.getAddToCartBtn(productsPage.listOfProducts.onesie).click();
        await productsPage.getAddToCartBtn(productsPage.listOfProducts.fleece).click();
        await productsPage.getAddToCartBtn(productsPage.listOfProducts.backPack).click();
    ...
    await browser.step("User navigates to the cart and verifies item has been added", async function () {
      await productsPage.cartIcon.click();
      await expect(cartPage.pageHeader).toHaveText(cartPage.headerText);
      await expect(cartPage.getCartItem(cartPage.listOfProducts.onesie)).toBeDisplayed();
      await expect(cartPage.getCartItem(cartPage.listOfProducts.fleece)).toBeDisplayed();
      await expect(cartPage.getCartItem(cartPage.listOfProducts.backPack)).toBeDisplayed();
    });

    await browser.step("User clicks on the checkout button. Chekout information page is displayed", async function () {
      await cartPage.checkoutBtn.click();
      await expect(cartPage.pageHeader).toHaveText(cartPage.infoPageHeaderText);
    });
    ...
    await browser.step(
      ...
      async function () {
        await cartPage.completeInfoForm(cartPage.firstName, cartPage.lastName, cartPage.zip);
        await cartPage.continueBtn.click();
        await expect(cartPage.pageHeader).toHaveText(cartPage.overviewPageHeaderText);
      }
    );

    await browser.step('User clicks on the "Finish"button. Order completion page is displayed', async function () {
      await cartPage.finishBtn.click();
      await expect(cartPage.completeHeaderTitle).toHaveText(cartPage.completeHeaderText);
    });
  });
});
